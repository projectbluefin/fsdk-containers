...
## Catalog-first image additions

The new image-catalog workflow starts with `catalog/<name>.yaml` and
`scripts/catalog.py`. Task 1 lands the schema, loader, and `base` record only;
later tasks will consume the catalog to generate the hand-authored image files.
Until that generation exists, keep the legacy element/README/Justfile wiring in
sync manually.
