...
pushes the refreshed refs back to the same branch. The workflow passes each
changed `elements/lab-runner/*.bst` path directly to `just bst`; the recipe
expects repository-relative element paths.
