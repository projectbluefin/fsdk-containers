...
The architecture-specific `ref:` values on remote sources are generated by
BuildStream and must stay synchronized with the version variable and URL.
Renovate's custom regex manager can update the version variable but cannot
calculate these hashes. The scheduled `track-bst-sources.yml` workflow runs
`just bst --no-interactive source track elements/<element>` for the affected
`lab-runner` elements, validates the changed element graph, and opens one PR
per element containing the version/ref update together.

Use BuildStream's `bst source track` command for source tracking; do not remove
sha256 refs to make automated updates easier.
