...
| `ghcr.io/projectbluefin/review-runtime` | — | Review-appliance runtime: python3, node, git, curl plus the POSIX text tools (diff, find, gawk, gzip, less, sed, tar). **Not yet published** — the first publish lands with the next successful `main` build, now that the `node/node.bst` build-root failure ([#289](https://github.com/projectbluefin/fsdk-containers/issues/289)) is fixed. Multi-arch: linux/amd64, linux/arm64. |
