...
worker="${repo_root}/elements/podman-vm/donate-clanker-worker.bst"
...
grep -Fq "ref: ${worker_commit}" "${worker}"
grep -Fq "path: elements/podman-vm/files/goose.yaml" "${config}"
grep -Fq "path: elements/podman-vm/files/local-agent-policy.md" "${config}"
grep -Fq 'opt/bluefin/goose/config/config.yaml' "${config}"
grep -Fq 'opt/bluefin/local-agent-policy.md' "${config}"
