...
class RenovateAtomicTests(unittest.TestCase):
    ...
    def test_refresh_workflow_recomputes_refs_for_bst_changes(self):
        ...
        paths = set(workflow[on_key]["pull_request"]["paths"])
        self.assertTrue(
            {"elements/*.bst", "elements/**/*.bst"}.issubset(paths),
            f"refresh-bst-refs.yml must watch both elements/*.bst and elements/**/*.bst: {paths}",
        )

        steps = workflow["jobs"]["refresh"]["steps"]
        self.assertTrue(
            any("source track" in step.get("run", "") and "bst" in step.get("run", "") for step in steps),
            "refresh-bst-refs.yml must run `bst ... source track` in a refresh step",
        )
